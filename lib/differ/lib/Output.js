...
function Output(diff, newVersion, oldVersion) {
    this.diff       = diff;
...
Output.prototype.addObject = function(obj, output, indention) {
    ...
    if (obj.newValue || obj.oldValue) {
        ...
        me.addBullet(me.newVersion, output, indention);

        indention += indentInc;

        me.addBullet(
            obj.key + ' is ' + (encoder ? encoder(obj.newValue) : obj.newValue),
            output,
            indention
        );
        ...
        me.addBullet(me.oldVersion, output, indention);

        indention += indentInc;

        me.addBullet(
            obj.key + ' is ' + (encoder ? encoder(obj.oldValue) : obj.oldValue),
            output,
            indention
        );
    }
