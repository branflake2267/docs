...
const args = require('yargs')
    .option({
        ...
        workspace : {
          required : true,
          type        : 'string',
          description : 'Stipulates the workspace directory for the site build configs and base output directory.',
          example     : 'node --max-old-space-size=4076 index create-app-html runGuides --workspace=~/git/docs/build_config_sencha --product=extjs --version=6.2.1 --cmdPath=../../sencha_cmd/sencha'
      },
...
if (canRun) {
    ...
    console.log(`The workspace directory is ${options.workspace}`);

    options._myRoot = resolveDirectory(options.workspace);

    console.log(`The workspace directory has been set to '${options._myRoot._myRoot}'`);
...
function resolveDirectory(dir) {
  var p = dir;
  if (dir.contains('~')) {
    p = Path.join(process.env.HOME, filepath.slice(1));
  }
  return p;
}
