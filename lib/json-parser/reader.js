...
function createTree () {
    ...
    glob((dir + '/' + '*-all-classes.json'), function(err, matches) {
        if (err) throw err
...
var tree = "[{ name: 'Item 1', children: [] },{ name: 'Item 2', children: [ " +
    "{ name: 'Sub Item 1', children: [] },{ name: 'Sub Item 2', children: [] }]}]";
    ...
    fs.readdir(dir,function(err,files) {
        ...
        var readFiles = function(index) {
            ...
            if (index == files.length) {
                ...
                if (junk.not(files[index]) && !files[index].includes('-all-classes.json')) {
