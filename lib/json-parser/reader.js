...
function parser(data){
    for ( var i=0; i<data.length; i++ ) {
        ...
        /*
        //Iterate through JSON object for text key
        //May want to do something like this instead of discretely assigning each node to a variable
        for (var key in item) {
            if (item.hasOwnProperty(key)) {
                if (key === "text") {
                    item[key] = marked(item[key] || '');
                }
                if (key === "items") {
                    // Process member text
                    for ( var j=0; i<item[key].length; j++ ) {
                        //
                        console.log(item[key][j].items);
                    }
                }
            }
        }*/
        ...
        var view = {
            name: name,
            altNames: item.alternateClassNames ? item.alternateClassNames.split(',').join('<br>') : '',
            mixins: item.mixins ? item.mixins.split(',').join('<br>') : '',
            requires: item.requires ? item.requires.split(',').join('<br>') : '',
            classText: scrubText(item.text),
        ...
        if(index == files.length){
            ...
            if(junk.not(files[index])) {
                fs.readFile(dir + "/" + files[index], {encoding: 'utf-8'}, function(err,json) {
                    ...
                    json = json.replace(/\{\s*@link(?:\s+|\\n)(\S*?)(?:\s+(.+?))?\}/g, function(match,link,text){
                        if(!text){
                            text = link;
                        }
