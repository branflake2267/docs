...
class Base {
    constructor (targets, options) {
            ...
            projects.forEach(function (project) {
                project.versions.forEach(function (pversion) {
                    if (pversion.version == pver) {
                        ...
                        shell.exec('sencha doxi build -p ' + pver + '.doxi.json combo-nosrc');
                        shell.exec('sencha doxi build -p ' + pver + '.doxi.json all-classes');
        ...
        } else if (parser == 'Source'){
            debug.error('Error: You must include a version');
            process.exit(1);
        } else if (!pver && parser != 'Diff') {
            debug.error('Error: You must include a version');
            process.exit(1);
        }
