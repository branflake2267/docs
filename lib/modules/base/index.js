...
class Base {
    constructor (options) {
        ...
        // TODO -- Add guide update for guide parser
        if (parser == 'Guide') {
        ...
        if (parser == 'Source' || options.needsSDK === true) {
            ...
            me.reconcileRepos(options, false, options.skip);
        }
    ...
    get productDir () {
        ...
        if (!dir) {
            let dest = me.destination;

            dir = me._productDir = me.hasVersions ? path.join(dest, me.getProductVersion()) : dest;
        }
    ...
    get docDir () {
        ...
        if (!dir) {
            let toolkit = me.toolkit;

            dir = me._docDir = path.join(me.productDir, (toolkit ? toolkit : 'api'));
        }
    ...
    getProductVersion (pversion) {
        pversion = pversion || this.version;
    ...
    prependHrefPath (tree, currPath, destPath) {
        let me = this;
        Utils.each(tree, function (item) {
            ...
            if ((item.href && !item.children) || (item.href && item.type === 'singleton') || (item.href=== '')) {
                ...
                myPath = path.relative(currPath, item.pathPrefix || destPath);
                item.preparedHref = (Utils.isEmpty(myPath) ? '.' : myPath) + '/' + item.href;
                //console.log(item.preparedHref);
                //if ((item.preparedHref.substr(item.preparedHref.length - 5) !== '.html') && item.href != '') {
                if ((item.preparedHref.substr(item.preparedHref.length - 5) !== '.html') && item.href != '') {
                    if (item.name === 'Sample Login App') {
                    }
                    if (item.name === 'Button') {
                    }
                    //item.preparedHref = item.preparedHref + '.html';
                    //item.preparedHref = path.relative(pathPrefix, item.href) + '.html';
                    //console.log(item.pathPrefix);
                    //item.preparedHref = path.join(path.relative(me.docDir, pathPrefix), item.href) + '.html';

                    //console.log(item.href);
                    //console.log(path.relative(currPath, item.pathPrefix || destPath));
                    //console.log(item.preparedHref + '.html');
                    item.preparedHref += '.html';
                }
            }
            if (item.children) {
                me.prependHrefPath(item.children, currPath, destPath);
            }
            if (item.compound && item.toolkits.length) {
                me.prependHrefPath(item.toolkits, currPath, destPath);
            }
        });
    }
     ...
     */
    reconcileRepos(options, cleanup, skip) {
        let me = this,
    ...
    createIndexPage (view) {
        let me = this,
            ...
            dest = me.productDir,
            homeoutput, treeview, treeoutput;

        if (me.hasGuides) {
            me.prependHrefPath(view.guideJson, me.productDir, path.join(me.productDir, 'guides'));
        }
        if (view.quickStartJson) {
            //me.prependHrefPath(quickStartTree, outputDir);
            me.prependHrefPath(view.quickStartJson, me.productDir, path.join(me.productDir, 'guides'));
        }

        treeview = {
            guideJson      : view.guideJson,
            quickStartJson : view.quickStartTree
        };

        treeoutput = treetemplate(treeview);
        ...
        wrench.copyDirSyncRecursive('./product-templates/images', path.join(dest, 'home-images'), {
            forceDelete: true
        });

        //wrench.copyDirSyncRecursive(__dirname + '/css/fonts', me.destination + '/css/fonts/', {
        wrench.copyDirSyncRecursive(__dirname + '/css/fonts', path.join(dest, 'css', 'fonts'), {
            forceDelete: true
        });

        //wrench.chmodSyncRecursive(me.destination + '/home-images/', '0755');
        wrench.chmodSyncRecursive(path.join(dest, 'home-images'), '0755');

        //fs.writeFileSync(me.destination + '/index.html', homeoutput, 'utf-8');
        fs.writeFileSync(path.join(dest, 'index.html'), homeoutput, 'utf-8');
    }
    ...
    beforeExecute () {
        let me          = this,
            destination = me.hasVersions ? path.join(me.destination, me.getProductVersion()) : me.destination,
            filemaploc  = path.join(me.docDir, 'src/map/filemap.json'),
