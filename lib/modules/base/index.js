                ...
                if (project.name == pname) {
                    ...
                    project.versions.forEach(function (pversion) {
                        if (pversion.version == pver) {
                            ...
                            // add clone if not exist
                            debug.info("Changing Directories to: " + repo );
                            shell.cd('../modules/' + repo);
                            ...
                            if (shell.cd(pversion.config) !== 0) {
                                //debug.error('Error: ' + pversion.config + ' does not exist.');
                            }
                            ...
                            shell.exec('sencha doxi build combo-nosrc');
                            shell.exec('sencha doxi build all-classes');

                            debug.info("Moving generated build files to docs input");
                            shell.mv(pversion.build, path + '/input/' + pversion.input);
