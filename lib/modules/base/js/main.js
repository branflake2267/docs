(function() {
    ...
    function runFiddleExample (wrap) {
        var editor = ace.edit(wrap.querySelector('.ace-ct').id),
            ...
            toolkit = pversion && pversion.indexOf('-') > -1 ? pversion.substr(pversion.indexOf('-') + 1) : false,
            codes  = [
                {
                    type : 'js',
                    name : 'app.js',
    ...
    function initHistory () {
        ...
        if (history && history.length) {
            ...
            ExtL.each(history, function (item) {
                var current = (item.product === product && item.pversion === pversion && item.text === pageName && item.path === path),
    ...
    function saveState() {
        ...
        if (isGuide || isApi) {
            ...
            ExtL.each(state.history, function (item, i, arr) {
                if (item.product === product && item.pversion === pversion && item.text === pageName && item.path === path) {
