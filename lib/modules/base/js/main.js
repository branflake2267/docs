(function() {
    ...
    function getSearchResultsCt () {
        ...
        if (!ct) {
            //posRef = compressed ? ExtL.get('mobile-input') :ExtL.get('searchtext');
    ...
    function onBodyClick (e) {
        ...
        if (e) {
            if (target.id != 'searchtext' && !isSearchNav) {
                hideSearchResults();
            } else {
    ...
    new TreeView(ExtL.productMap, 'product-tree', homePath);
    ...
    function highlightTypeMenuItem() {
        ...
        for (; i < len; i++) {
            ...
            if (item.offsetHeight + 10 && (itemTop < memberTypesBottom + 1)) {
                activeItem = item;
            }
        }
