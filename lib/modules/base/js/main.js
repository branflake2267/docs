(function() {
   ...
   function toggleExamples (collapse) {
    ...
    function showMultiSrcPanel (e) {
        e = e || window.event;
        var target = e.target || e.srcElement,
            multi = target.parentNode.querySelector('.multi-src-picker');

        if (multi) {
            ExtL.addCls(multi, 'show-multi')
        }
    }
    ...
    function hideMultiSrcPanel () {
        /*var multi = ExtL.get('multi-src-picker');

        if (multi) {
            ExtL.removeCls(multi, 'show-multi')
        }*/
        ExtL.each(ExtL.fromNodeList(document.querySelectorAll('.multi-src-picker')), function (item) {
            ExtL.removeCls(item, 'show-multi');
        });
    }
