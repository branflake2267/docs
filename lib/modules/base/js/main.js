(function() {
    var hasClassTemplate = window.isClassTemplate || false,
        ...
        guideTree, tree, masterSearchList, apiSearchRecords, guideSearchRecords, currentPage, eventsEl;
    ...
    function getSearchResultsCt () {
        var size = getViewportSize(),
            ...
            hasApi = ExtL.get('apiTab').offsetHeight,
            hasGuide = ExtL.get('guideTab').offsetHeight,
            searchInput, searchInputBox, top, cn;
    ...
    function loadApiSearchPage (page) {
        ...
        /*if (apiCt.childNodes.length) {
            ...
            if (apiSearchRecords.length > pageSize) {
                apiCt.appendChild(ExtL.createElement({
                    "class": 'search-results-nav',
                    html: (pageSize * page - pageSize + 1) + ' - ' + (pageSize * page) + ' of ' + apiSearchRecords.length,
                    cn: [{
                       "class": 'search-nav-first' + ((page === 1) ? ' disabled' : ''),
                        ...
                        "class": 'search-nav-forward' + ((apiSearchRecords.length <= page * pageSize) ? ' disabled' : ''),
                        html: '►'
                    }, {
                        "class": 'search-nav-last' + ((apiSearchRecords.length <= page * pageSize) ? ' disabled' : ''),
                        html: '»'
                    }]
                }));
            }
    ...
    function showSearchResults (page) {
        ...
        ExtL.addCls(getSearchResultsCt(), 'show-search-results');

        //ExtL.applyStyles(apiTab, 'display:' + (apiVisible ? 'block' : 'none') + ';');
        //ExtL.applyStyles(guideTab, 'display:' + (guideVisible ? 'block' : 'none') + ';');
    ...
    function onResultsCtClick (e) {
        ...
        if (ExtL.hasCls(target, 'search-nav-first') && !ExtL.hasCls(target, 'disabled')) {
            ...
            if (ExtL.up(target, '#api-search-results')) {
                loadApiSearchPage(currentPage - 1);
            }
            if (ExtL.up(target, '#guide-search-results')) {
                loadGuideSearchPage(currentPage - 1);
            }
    ...
    //ExtL.applyStyles(guideTab, 'display:' + (ExtL.treeDataGuide ? 'inline-block' : 'none') + ';');
    ...
    function fetchState(skipSave, returnOnly) {
        ...
        if (!state) {
            if (isApi) {
                ...
                if (guideTab.offsetHeight) {
                    ExtL.addCls(guideTab, 'active-tab');
                    if (apiTree) {
                        ExtL.addCls(apiTree, 'hide');
                    }
                } else if (apiTab.offsetHeight) {
                    ExtL.addCls(apiTab, 'active-tab');
                    if (guideTree) {
                        ExtL.addCls(guideTree, 'hide');
                    }
