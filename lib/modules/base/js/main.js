(function() {
    ...
    var filter = ExtL.createBuffered(function (e, target) {
        ...
        ExtL.each(classmembers, function (member) {
            var parent = member.parentNode,
                ...
                params = ExtL.fromNodeList(member.querySelectorAll('params-list'));
            ...
            if (!value.length) {
                ...
                ExtL.removeCls(member, 'member-temp-expanded');
                ...
                if (parent.id === 'sass-mixins-ct') {
                    ...
                    if (params.length) {
                        ExtL.each(params, function (paramEl) {
                            var name = paramEl.id.substr(paramEl.id.indexOf('--') + 2),
                                paramParent = paramEl.parentNode;
                    ...
                    ExtL[paramMatch ? 'addCls' : 'removeCls'](member, 'member-temp-expanded');
                }
            }
        });
            ...
            ExtL.each(searchIndex, function (i, cls) {  // iterate over each class object
                ...
                ExtL.each(cls, function (key, obj) {    // inspect each member - could be the class name, alias, or a class member object
                    ...
                    if (key === 'n') {                  // this is the class name
                        ...
                        composite = key.substr(0, 2) + cls.n + '.' + key.substr(2);
    ...
    function searchFilter(e){
        ...
        if (e.type === 'keydown' && value.length) {
        ...
        if (!value.length || (ExtL.isIE8() && value.length < 2)) {
            hideSearchResults();
            return;
        }
    ...
    function loadGuideSearchPage (page) {
        ...
        for (;i < len; i++) {
            ...
            if (item) {
                ...
                href = 'guides/' + guideSearchUrls[item.r] + '.html';
    ...
    ExtL.bindReady(function () {
        ...
        if (ExtL.get('guide-tree').firstChild) {
        ...
        productTree = new TreeView(null, 'product-tree', homePath);
        ExtL.get('product-tree').myTree = productTree;
        //productId = product + '-' + pversion.replace(/\./g, '');
        productId = product;
        if (hasVersions && pversion) {
            productId += '-' + pversion.substring(0, pversion.indexOf('-'));
        }
   ...
   function toggleExamples (collapse) {
    ...
    function positionProductMenu () {
        ...
        btnBox = (btn) ? btn.getBoundingClientRect() : 0;
    ...
    var resizeHandler = ExtL.createBuffered(function () {
        ...
        /*if (width < 950 && isLanding && !ExtL.hasCls(prodTree, 'hide')) {
            toggleProductMenu()
        }
    ...
    function saveState() {
        ...
        if (apiTab) {
            activeNavTab = ExtL.hasCls(apiTab, 'active-tab') ? 'apiTab' : 'guideTab';
        }
    ...
    function fetchState(skipSave, returnOnly) {
        ...
        if (!state || ExtL.isEmpty(state.activeNavTab)) {
            if (isApi && apiTab && apiTree) {
                ExtL.addCls(apiTab, 'active-tab');
                ExtL.removeCls(apiTree, 'hide');
                if (guideTree) {
                    ExtL.addCls(guideTree, 'hide');
                }
            } else if (isGuide && guideTab && guideTree) {
                ...
                if (hasApi && apiTab && (apiTab.offsetHeight || apiTab.clientHeight)) {
                    toggleNavTab('apiTab');
                } else if (hasGuides && guideTab && (guideTab.offsetHeight || guideTab.clientHeight)) {
