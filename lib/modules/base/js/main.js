(function() {
    ...
    var filter = ExtL.createBuffered(function (e) {
        ...
        ExtL.each(classmembers, function (member) {
            var parent = member.parentNode,
                ...
                params = ExtL.fromNodeList(member.getElementsByClassName('params-list'));
            ...
            ExtL.each(searchIndex, function (i, cls) {  // iterate over each class object
                ...
                ExtL.each(cls, function (key, obj) {    // inspect each member - could be the class name, alias, or a class member object
                    ...
                    if (key === 'n') {                  // this is the class name
                        ...
                        //console.log(ExtL.format(itemTpl, i, key.substr(0, 2) + cls.n + '.' + key.substr(2)));
                        composite = key.substr(0, 2) + cls.n + '.' + key.substr(2);
                        list += ExtL.format(itemTpl, i, composite);
                        //console.log(cls.n, key);
                        //cls[]
    ...
    function applyAceEditors () {
        ...
        ExtL.each(invisibles, function (block) {
            ExtL.removeCls(ExtL.get(block), 'invisible');
        });
    }
    ...
    function fetchState(skipSave, returnOnly) {
        ...
        if (!state) {
            if (isApi && apiTab && apiTree) {
                ...
                if (apiTab && (apiTab.offsetHeight || apiTab.clientHeight)) {
                    ExtL.addCls(apiTab, 'active-tab');
                    if (guideTree) {
                        ExtL.addCls(guideTree, 'hide');
                    }
                } else if (guideTab && (guideTab.offsetHeight || guideTab.clientHeight)) {
                    ExtL.addCls(guideTab, 'active-tab');
                    if (apiTree) {
                        ExtL.addCls(apiTree, 'hide');
                    }
