                ...
                var container  = document.getElementById(self.node),
                    ...
                    renderLeaf = function(item) {
                        ...
                        /*if (window.location.href.indexOf("guide") > -1) {
                            // need to count backslashes here to determine relative path back
                            guideTree = "./";
                            apiTree = "../";
                        } else {
                            guideTree = "./guides/";
                        ...
                        if ((!item.children) || (item.name == item.className) || (item.name == item.path)) {
                            if (item.className) {
                                //console.log('i guess it was item.className', apiTree + item.className + '.html', apiTree + prefix + item.className + '.html');
                                //text.href = apiTree + prefix + item.className + '.html';
                                text.href = prefix + item.className + '.html';
                            } else {
                                if (item.link) {
                                    //console.log('I guess it was item.link', item.link);
                                    text.href = item.link;
                                    text.target = '_blank';
                                } else {
                                    //console.log(guideTree + prefix + item.path + '.html');
                                    //text.href = guideTree + prefix + item.path + '.html';
