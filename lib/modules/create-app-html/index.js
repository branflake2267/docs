...
class HtmlApp extends AppBase {
     ...
     */
    run () {
        this.log("create-app-html Starting run()");

        const dt = new Date();
        
        //this.testGit();
        
        this.log("\tRunning super()");

        super.run()
        .then(this.outputProductHomePage.bind(this))
        .then(this.outputMainLandingPage.bind(this))
        .then(() => {
            //this.log('ALL TOLD:', this.getElapsed(dt));
            this.concludeBuild();
        })
    ...
    processLandingDataObject (data) {
        ...
        data.cssPath     = Path.relative(options._myRoot+ "/build/output", this.cssDir);
        data.jsPath      = Path.relative(options._myRoot+ "/build/output", this.jsDir);
        data.imagesPath  = Path.relative(options._myRoot+ "/build/output", this.imagesDir);
        data.myMeta      = this.getLandingMetaData(data);

        this.log("###########");
        this.log("outputProductDir=" + outputProductDir);
        this.log("resourceDir=" + this.resourcesDir);
        this.log("apiDir=" + this.apiDir);
        this.log("assetsDir=" + this.assetsDir);
        this.log("rootPath=" + this.rootPath);
        this.log("outputDir=" + outputDir);
        this.log("options._myRoot=" + options._myRoot);
        this.log("processLandingDataObject: data.jsPath=" + data.jsPath);
        this.log("relative path ### jsDir rel path=" + Path.relative(options._myRoot + "/build/output", this.jsDir));
    ...
    outputMainLandingPage () {
        this.log("~~~ output main: landing page: options.outputDir=" + this.options.outputDir);
        ...
        return new Promise((resolve, reject) => {
            let { options } = this,
                ...
                dest = Path.join(options._myRoot, dest);

            let data = Fs.readJsonSync(tplPath);
            data     = Object.assign(data, options);
            data     = Object.assign(data, options.prodVerMeta);

            data.rootPath       = '..'; // This is the root, the start of the guides
            data.contentPartial = '_product-home';
