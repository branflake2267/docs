        ...
        if (newItems && oldItems) {
                ...
                if (newMatch.ignore) {
                    ...
                    if (newMatch.name) {
                        let oldMatch = Utils.getMatch('name', newMatch.name, oldItems),
                            ...
                            isPrivate = me.isPrivateAccess(newMatch),
                            arr, obj;
            ...
            oldItems.forEach(function(oldMatch) {
                let newMatch = Utils.getMatch('name', oldMatch.name, newItems),
                    isPrivate = me.isPrivateAccess(oldMatch),
                    arr, obj;
        ...
        if (oldObj) {
            tests.forEach(function(test) {
                ...
                if (!info && newVal !==oldVal) {
                    ...
                    info.type     = newObj.$type;
                    info.isPrivate= me.isPrivateAccess(newObj);
                }
            });
                ...
                newObj.items.forEach(function(newItem) {
                    ...
                    if (oldItem) {
                        ...
                        info = me.addChildChanges(info, 'modified', newObj.name, 'added', [{
                            name: newItem.name,
                            action: 'added',
                            isPrivate: me.isPrivateAccess(newItem),
                            type: newItem.$type
                        }]);
                    }
                });
                ...
                oldObj.items.forEach(function(oldItem) {
                    ...
                    if (!newItem) {
                        info = me.addChildChanges(info, 'modified', newObj.name, 'removed', [{
                            name: oldItem.name,
                            action: 'removed',
                            isPrivate: me.isPrivateAccess(oldItem),
                            type: oldItem.$type
                        }]);
                    }
                });
            ...
            info = {
                ...
                name     : newObj.name,
                type     : newObj.$type,
                isPrivate: me.isPrivateAccess(newObj)
            };
        }
