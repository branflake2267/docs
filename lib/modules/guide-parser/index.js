...
class Guide extends Base {
    ...
    run () {
        let me           = this,
            ...
            hometemplate = configs.guides.homeTemplate || options.hometemplate,
        ...
        if (me.targets[0] === 'tree') {
            ...
            new compressor.minify({
                ...
                fileIn  : [options.extl, options.treeview],
        ...
        fs.writeFileSync(output + 'js/guide-tree.js', 'ExtL.treeData = ' + stringfiedTree + ';', 'utf-8');
        ...
        contents.forEach(function(content) {
            ...
            if (copyImages) {
                for (i=0; i < versions.length; i++) {
                    if (me.syncImages(node, versions[i], isRoot) === true) {
                        break;
                    }
                }
            }
    ...
    syncImages (node, version, isRoot) {
        let options = this.options,
            guidesInput = this.guidesInput,
            imagesPath = guidesInput + '/' +  version + (isRoot ? '' : ('/' + node.path)) + '/images',
            imgs = options.imgs,
            output = this.output;
