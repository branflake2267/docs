...
class Guide extends Base {
    ...
    run () {
        let me           = this,
            ...
            template     = configs.guides.template,
        ...
        me.template = fs.readFileSync(template, 'utf-8');
        ...
        for (i=0; i<nodes.length; i++) {
            ...
            if (nodes[i]) {
                let obj = {
                    url : {
                        loc: this.docroot + 'guides/' + nodes[i].node.path + '.html'
                    }
                };
    ...
    createLink (link, namespace, text) {
        let flinks = this.configs.frameworks[link];
    ...
    parseAPILinks (html) {
        ...
        html = html.replace(/\[\[(.*?):(.*?)\s(.*?)]]/gi, function(match, link, namespace, text) {
