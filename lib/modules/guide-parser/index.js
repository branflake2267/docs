...
class Guide extends Base {
    ...
    run () {
        ...
        if (me.hasApi) {
            me.prependHrefPath(apiJson, me.destination + 'guides/');
        }
    ...
    parseContents (contents, parent, paths, versions, nodes) {
        ...
        contents.forEach(function(content) {
            let nodePath = paths ? paths + '/' + content.slug : content.slug,
                //guideDir = path.join(me.output, node.path.substr(0, node.path.lastIndexOf('/')))
            ...
            if (content.children) {
                ...
                if (content.link) {
                    ...
                    //node.href = path.relative(guideDir, me.destination + '/guides') + '/' + me.guides.output + '/' + nodePath + '.html';
