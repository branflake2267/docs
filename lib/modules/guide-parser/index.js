...
class Guide extends Base {
    ...
    //createLink (link, namespace, text, node) {
    createLink (namespace, className, memberName, text, node, product) {
        let prodConfig = this.getProductConfig(product),
            outputPath = prodConfig.destination,
            href;
        ...
        href = path.relative(node.guideDir, outputPath) + '/' + className + '.html';
        ...
        //console.log(this.destination, ' * ', node.guideDir);
        //console.log(path.relative(node.guideDir, this.destination) + '/' + namespace);
    ...
    parseAPILinks (html, node) {
        ...
        html = html.replace(/\[{2}([a-z]+):([a-z.\-#]+)\s?([a-z'.()[\]\-\s]*)\]{2}/gim, function(match, product, namespace, text) {
            ...
            // replace the hash and what follows with some meaningful stuff
            /*href = hash.replace(/^(cfg-|property-|static-property-|method-|static-method-|event-|css_var-S-|css_mixin-)?([a-zA-Z0-9$-_]+)/, function (match, type, memberName) {
                let linkObj = me.findMemberType(className, memberName);
                console.log(linkObj);
                return '';
            });

            return me.createLink(link, namespace, text, node);*/
