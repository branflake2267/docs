...
class Guide extends Base {
    ...
    parseContents (contents, parent, paths, versions, nodes, skipImages) {
        ...
        contents.forEach(function(content, i) {
            let nodePath = paths ? paths + '/' + content.slug : content.slug,
                ...
                guideDir = path.join(me.docDir, nodePath),
            ...
            if (content.children) {
                ...
                if (content.link) {     // this is an external link node only
                    ...
                    //node.href = path.join(me.docDir, nodePath);
                    node.href = nodePath;
    ...
    syncImages (node, versions, isRoot) {
        let me = this,
            ...
            //output = me.output,
            output = me.docDir,
            //outPath = output + (isRoot ? '' : node.path),
        ...
        Utils.each(vers.reverse(), function (ver) {
            ...
            me.copyDirectory(srcPath + '/images', outPath + '/images/');
            me.copyDirectory(srcPath + '/resources', outPath + '/resources/');

            /*// Check to see if images director exists in source folder
            // Copy if so
            if (fs.existsSync(imagesPath)) {
                shell.mkdir('-p', outPath);
                fs.copy(image sPath, outPath);
    ...
    parseNode (node, versions, guideJsonObj, apiJson) {
        ...
        //me.prependHrefPath(tree, path.join(guideDir, node.path), guideDir);
        ...
        //me.prependHrefPath(productTree, guideDir);
