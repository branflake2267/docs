...
class Guide extends Base {
    get productIndex () {
        return {
            extjs: {
                ...
                toolkits: ['6'],
                defaultToolkit: 'classic',
                hasVersions: true,
                '3': {
                    prefix: 'http://docs.sencha.com/',
                    suffix: '/#!/api/',
                    link: 'http://docs.sencha.com/{prod}/{ver}/#!/api/{page}'
                },
                '4': {
                    prefix: 'http://docs.sencha.com/',
                    suffix: '/#!/api/',
    ...
    /**
     * This is a supporting method used by the getNormalizedProductAndVersion method.  A
     * product is passed in and the formal product name is returned.
     * @param {String} prod The product name passed in the [[product:]] formatted link.
     * @return {String} The formal product name
     */
    getNormalizedProductName (prod) {
        let list = {
            ext: 'extjs',
            extjs: 'extjs',
            classic: 'extjs',
    ...
    getNormalizedProductAndVersion (product) {
        ...
        return {
            productLiteral: prod,
            product: this.getNormalizedProductName(prod),
            version: ver
        };
    }
         ...
         fs.stat(imagesPath, function(err) {
             if(!err) {
                 ...
                 if(!fs.existsSync(output + (isRoot ? '' : node.path))) {
                     shell.mkdir(output + (isRoot ? '' : node.path));
                 }
