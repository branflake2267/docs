...
class JsonParser extends ClassTree {
    ...
    fileExists (item) {
        ...
        let file = this.input + item + '.json';

        try {
            let exists = fs.statSync(file).isFile();
                ...
                if (memberTypeGroups && memberTypeGroups.length) {
                    ...
                    if (configs) {
                        ...
                        configs.items.forEach(function (config) {
                            ...
                            if (config.setter) {
                                me.removeAt(methods.items, methods.items.indexOf(config.sgetter));
                            }
