...
class JsonParser extends ClassTree {
    ...
    parser (datas, outputDir) {
        let me   = this,
            ...
            fileMap = JSON.parse(fs.readFileSync(this.destination + 'src/map/filemap.json', 'utf-8')),
            classarr  = [],
            searchIndex = {};
                        ...
                        if (members && members.length) {
                            members.forEach(function(member, i) {
                                let memberFileLoc = me.getMemberLoc(member),
                                    ...
                                    fileLink      = me.inObject(cleanLink,fileMap) || name + '.html';
                                ...
                                member.srcLink = '<div class="viewSource">' +
                                                 '<a target="_blank" href="src/' +
                                                    fileLink + '#' + fileLink.replace('.html','') + '-' + member.$type + '-' + member.name + '">' +
                                                 'view source</a></div>';
