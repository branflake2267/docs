...
class JsonParser extends ClassTree {
    ...
    fileExists (item) {
        ...
        //return !!(this.fileArray.indexOf(thisfile) != -1);
    ...
    getFiddleId (version) {
        let projects = this.projectConfigs.products,
            fiddleId;
    ...
    parser (classNames, outputDir) {
        ...
        if (classNames && classNames.length) {
            ...
            let root = xmlbuild.create('urlset').att("xmlns","http://www.sitemaps.org/schemas/sitemap/0.9"),
                ...
                sitemap, smOutput = [], xmlString, hasIndex;
            ...
            if (me.hasGuides) {
                guideJson = new GP(me.options).getGuideJson(true).tree;
            }
            ...
            Utils.each(classNames, function(name) {
                ...
                me.prependHrefPath(productMapJson, outputDir);
            ...
            fs.writeFileSync(outputDir + '/js/searchIndex.js', 'var searchIndex = ' + JSON.stringify(me.getSearch()) + ';', 'utf-8');
        }
    }
    ...
    buildConfig(config, obj) {
        ...
        config.getter = obj.instanceMethods['get' + Utils.capitalize(config.name)];
        config.setter = obj.instanceMethods['set' + Utils.capitalize(config.name)];
    ...
    processMemberTypeGroups (memberType, obj) {
        ...
        obj.properties = (type === 'properties') ? memberType : null;
        obj.methods = (type === 'methods') ? memberType : null;
        obj.staticProperties = (type === 'static-properties') ? memberType : null;
        obj.staticMethods = (type === 'static-methods') ? memberType : null;
        ...
        // store these so we can concatenate the required and optional configs
        obj.configs = (type === 'configs') ? memberType : null;
    ...
    processMember(member, i, obj, type) {
        ...
        if (member.items) {
            ...
            member.items.forEach(function(item) {
                ...
                if (item.items) {
                    //me.processMember(item, i, topLoopIndex, obj, type, searchIndex);
