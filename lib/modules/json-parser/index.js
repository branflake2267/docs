...
class JsonParser extends ClassTree {
    ...
    beforeExecute (inputDir, output, configs, fileArray) {
        ...
        this.smTemplate = fs.readFileSync(__dirname + '/sitemap.hbs', 'utf-8');
        this.title = title;
        this.footer = footer;
        this.headhtml = headhtml;
        this.fileArray = fileArray;
        this.docroot = configs.docroot;
    ...
    parser (datas, outputDir) {
        ...
        if (datas && datas.length) {
            ...
            var root = xmlbuild.create('urlset')
                        .att("xmlns","http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd");
            ...
            for(var i = 1; i <= smOutput.length; i++) {
                if (smOutput[i]) {
                    ...
                    root.ele('loc', this.docroot + smOutput[i].fileName);
                    root.ele('lmod', this.date);
                }
            }

            var xmlString = root.doc().end({ pretty: true, indent: '  ', newline: '\n' });
