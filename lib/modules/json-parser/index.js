...
class JsonParser extends ClassTree {
    ...
    beforeExecute (input, output) {
        let options     = this.options,
            ...
            destination = (configs.destination.value) ? configs.destination.value : output,
            template    = (configs.template)     ? configs.template     : options.template,
            compress    = (configs.compress)     ? configs.compress     : options.compress, //true to compress js/css files, false to only concatenate
            stylesheet  = (configs.stylesheet)   ? configs.stylesheet   : options.stylesheet;
