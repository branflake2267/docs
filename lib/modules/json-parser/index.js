...
class JsonParser extends ClassTree {
    ...
    /*get classMapRaw () {
        let me = this,
            raw = me._processedClassesRaw;

        if (!raw) {
            let temp = Object.assign({}, super.classMapRaw);

            Utils.each(temp, function (key, value) {
                temp[key] = value.replace(imgRe, function(match, img) {
                    return "<img src='images/"+ img +"'/>";
                });

                temp[key] = value.replace(linkRe, function(match, link, text) {
                    link = link.replace('!','-');
            ...
            raw = me._processedClassesRaw = temp;
        ...
        if (files) {
            files.forEach(function (file) {
                if (junk.not(file) && !file.includes('-all-classes')) {
                    let json  = JSON.parse(fs.readFileSync(input + file + '.json', 'utf-8')),
