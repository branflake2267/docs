...
class JsonParser extends ClassTree {
    ...
    processMemberTypeGroups (memberType, topLoopIndex, obj, searchIndex) {
        ...
        if (!me.headerLink) {
            let headerFile = (obj.files[0] && obj.files[0].indexOf('../') > -1) ? obj.files[0].replace(/\.\.\//g,'') : obj.files[0];
            me.headerLink = me.inObject(headerFile,me.fileMap) || name + '.html';
        }
