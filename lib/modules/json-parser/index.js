...
handlebars.registerPartial('headerHome', (function() {
    var src = '<html>' +
    '<head>' +
        '<title>{{{title}}}</title>' +
        '<meta charset="UTF-8">' +
        ...
        '<div id="header">' +
        '    <h1>{{{title}}}</h1>' +
        '<div class="search"><form><input type="text" placeholder="search" id="searchtext"></form></div>' +
        ...
        '    </div>' +
        '    <div class="toolbarButton">' +
        '    <a href="http://examples.sencha.com/extjs/6.0.0/examples/" target="_blank">examples</a>' +
        '    <div class="nav-btn-mask"></div>' +
        '    </div>' +
        '    </div>' +
        '    </div>';
...
/**
 * The {{#footer-home}} helper to generate class member markup
 */
handlebars.registerPartial('footerHome', (function() {
    var src = '<div id="footer">{{{footer}}} Generated on {{{date}}}</a></div>' +
              '</body>' +
 ...
 */
handlebars.registerPartial('processMember', (function(m) {
    var src = '{{#unless hide}}' +
        '<div class="classmembers member{{#access}}-{{{.}}}{{/access}} {{#if static}}isStatic{{else}}isNotStatic{{/if}} {{#is $type "config"}}{{#if required}}isRequired{{else}}isNotRequired{{/if}}{{/is}} {{#isInherited}}isInherited{{/isInherited}} {{#fromObject}}fromObject{{/fromObject}}" data-member-name="{{{name}}}" {{#accessor}} data-accessor="true"{{/accessor}}{{#inheritdoc}} data-inherited="true"{{/inheritdoc}}>' +
            ...
            '{{/if}}' +
            '{{#autoGetter}}' +
                '<div class="classmembers member accessor-method isGetter" data-member-name="get{{capitalize name}}" data-accessor="true">' +
                    '<h2 id="{{{$type}}}-get{{capitalize name}}" class="member-header">' +
                        '<a href="#{{{$type}}}-get{{capitalize name}}" class="member-name">' +
                            'get{{capitalize name}}' +
                        '</a>' +
                        '<span class="memberType">{{{type}}}</span>' +
                    '</h2>' +
                    '<p>Returns the value of {{name}}</p>' +
                    '<h3 class="detail-header">Returns</h3>' +
                    '<p><span class="memberType">{{{type}}}</span></p>' +
                '</div>' +
            '{{/autoGetter}}' +
            ...
            '{{/if}}' +
            '{{#autoSetter}}' +
                '<div class="classmembers member accessor-method isSetter" data-member-name="set{{capitalize name}}" data-accessor="true">' +
                    '<h2 id="{{{$type}}}-set{{capitalize name}}" class="member-header">' +
                        '<a href="#{{{$type}}}-set{{capitalize name}}" class="member-name">' +
                            'set{{capitalize name}}' +
                        '</a>' +
                        ' <span class="params-list">({{name}})</span>' +
                    '</h2>' +
...
class JsonParser extends ClassTree {
    ...
    parser (datas, outputDir) {
        ...
        if (datas && datas.length) {
                        ...
                        if (members && members.length) {
                            members.forEach(function(member, i) {
                                ...
                                if (!member.from) {
                                    ...
                                    //searchList += searchIndexI + '-' + typeRef[type] + '.' + member.name + '`';
                                }
                            });
                        }
                    });
            ...
            //fs.writeFileSync(me.destination + 'js/searchIndex.js', 'var searchIndex = ' + JSON.stringify(searchIndex) + ',searchList = "' + searchList + '"', 'utf-8');
            fs.writeFileSync(me.destination + 'js/searchIndex.js', 'var searchIndex = ' + JSON.stringify(searchIndex) + ';', 'utf-8');

            new compressor.minify({
                type    : 'no-compress',
                fileIn  : [me.destination + 'js/app.js', me.destination + 'js/searchIndex.js'],
