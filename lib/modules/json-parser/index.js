...
JsonParser.prototype.run = function() {
    ...
    /**
     * copy the stylesheet to output/styles.css
     * (doesn't matter what input name is, will always be styles.css in output)
     */
    fs.createReadStream(stylesheet).pipe(fs.createWriteStream(output + 'css/styles.css'));

    /**
     * Copy treeview css/js
     */
    fs.createReadStream(__dirname + '/css/treeview.css').pipe(fs.createWriteStream(output + 'css/treeview.css'));
    fs.createReadStream(__dirname + '/js/treeview.js').pipe(fs.createWriteStream(output + 'js/treeview.js'));
...
/* Commenting out until I can figure out how the restructure changed linkages
   It seems that we're getting a bunch of link issues
        - .html.html" (page links)
        - \Ext.grid.Panel\ (meta information)
        - Types were broken in addition to their delimiter
        - etc

        I'll be sure to normalize it back into createLink once I get the logic sorted out
...
JsonParser.prototype.splitCommas = function(splitstring) {
    ...
    if (splitstring && splitstring.includes(',')) {
        ...
        splitstring.forEach(function(str) {
            ...
            linkstring += "<a href='"+str+".html'>"+str+"</a><br>";
        });
                                ...
                                if (typelinks.includes('/')) {
                                    ...
                                    if (typelinks && typelinks.length) {
                                        typelinks.forEach(function(typelink, idx, arr) {
                                            //use RegExp to do in one call?
                                            var safelinks = typelink.replace('[]', '').replace('...', '');

                                            //container.type += me.createLink(safelinks, typelink);
                                            container.type += "<a href='"+safelinks+".html'>"+typelink+"</a>";
                                    ...
                                    container.type = "<a href='"+typelinks+".html'>"+typelinks+"</a>";
                                }
                            }
