...
JsonParser.prototype.createTree = function() {
    ...
    return new Tree({
        nodeParser : function(node, className) {
            if (className.split('.').pop() === node.name) {
                node.name = me.createLink(className, node.name);
            }
        ...
        if (files) {
            files.forEach(function(file) {
                ...
                if (junk.not(file) && !file.includes('-all-classes')) {
                    ...
                    json = json.replace(linkRe, function(match, link, text) {
                        if (!text) {
                            text = link;
                        }
