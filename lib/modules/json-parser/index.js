...
class JsonParser extends ClassTree {
    ...
    parser (datas, outputDir) {
        let me   = this,
            ...
            fileMap = JSON.parse(fs.readFileSync(__dirname + '/filemap.json', 'utf-8')),
            classarr  = [],
            searchIndex = {};
        ...
        if (datas && datas.length) {
                        ...
                        if (members && members.length) {
                            members.forEach(function(member, i) {
                                let memberFileLoc = me.getMemberLoc(member),
                                    ...
                                    fileLink      = me.inObject(memberFile.replace(/\.\.\//g,''),fileMap) || name + '.html';
                                ...
                                member.srcLink = '<div class="viewSource">' +
                                                 '<a target="_blank" href="src/' +
                                                    fileLink + '#' + name + '-' + member.$type + '-' + member.name + '">' +
                                                 'view source</a></div>';
            ...
            new compressor.minify({
                type    : 'no-compress',
                fileIn  : [me.destination + 'js/home.js', me.destination + 'js/searchIndex.js'],
                fileOut : me.destination + '/js/home.js'
            });
        }
    }
