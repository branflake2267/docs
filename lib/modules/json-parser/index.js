...
class JsonParser extends ClassTree {
    ...
    filesToClassnames (files) {
        let outFiles = [],
            i = 0,
            len, file, isPrimitive;
        ...
        if (files) {
            ...
            for (; i < len; i++) {
                file = files[i];
                isPrimitive = !!(file.indexOf('primitives') > -1);

                if (!isPrimitive) {
                    file = file.replace(/(.)*\/classic\/classic\/src\/|(.)*\/classic\/theme(.)*\/sass\/var\/|(.)*ext\/packages\/(.)*\/src\//, 'Ext/');
                    file = file.replace(/.js$|.scss$/, '').replace(/\//g, '.');
                }
                outFiles.push(file);
            }
        }
    ...
    parser (datas, outputDir) {
        ...
        if (datas && datas.length) {
            ...
            datas.forEach(function(data) {
                if (JSON.parse(data).global.items[0].name === 'Ext.grid.Panel') {
                    //debugger;
                }
                let obj       = JSON.parse(data),
                    ...
                    types     = cls.$type,
                    files     = me.filesToClassnames(obj.files),
                    priority  = {
                        configs: 0,
                        properties: 1,
                        "static-properties": 2,
                    ...
                    members.forEach(function(member) {
                        ...
                        if (containers && containers.length) {
                            containers.forEach(function(container) {
                                ...
                                // the source class where this name originated is the
                                // file at the index indicated by the first number in the
                                // src's name key
                                if (container.src.name) {
