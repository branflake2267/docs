(function() {
    ...
    function setTypeNavAndHeaderVisibility () {
        ...
        if (totalCount === 0) {
            ExtL.addClass(document.body, 'no-visible-members');
        } else {
            ExtL.removeClass(document.body, 'no-visible-members');
        }
    };
    ...
    function toggleRelatedClassesCt(collapse) {
        var btn = ExtL.get('related-classes'),
            ...
            collapsed = ExtL.hasClass(body, collapsedCls),
            collapse = (collapse === true || collapse === false) ? collapse : !collapsed;

        if (collapse) {
            ExtL.addClass(body, collapsedCls);
        } else {
            ExtL.removeClass(body, collapsedCls);
        }
    ...
    function saveState() {
        ...
        state.collapseRelatedClasses = ExtL.hasClass(document.body, 'related-collapsed');
