(function() {
    ...
    function setTypeNavAndHeaderVisibility () {
        ...
        for (i = 0; i < headersLen; i++) {
            ...
            count = 0;
            for (j = 0; j < len; j++) {
                if (els.item(j).className.indexOf('be-hidden') < 0) {
                    count++;
                    hasVisible = true;
                }
            }
    ...
    function showMemberTypeMenu (e) {
        ...
        if (targetId === 'methods' || targetId === 'properties') {
            eligMembers.sort(function (a, b) {
                var aStr = ExtL.trim(a.firstChild.textContent || a.firstChild.innerText),
                    bStr = ExtL.trim(b.firstChild.textContent || b.firstChild.innerText);
