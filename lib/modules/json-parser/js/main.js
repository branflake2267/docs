(function() {
    ...
    function getSearchList () {
        ...
        if (!list) {
            list = '';
            ExtL.each(searchIndex, function (i, cls) {
                ExtL.each(cls, function (key, obj) {
                    if (key === 'n') {
    ...
    function searchFilter(e){
        /*var searchdata = JSON.parse(ExtL.searchdata),
            value = ExtL.trim(e.target.value).toLowerCase(),
        ...
        // Look for search values within classnames specifically
        // Then add to the results array
        for (var i = 0; i < searchdata[0].length; i++) {
            n = searchdata[0][i].toLowerCase().indexOf(value);
            if (!n) {
                best.push(searchdata[0][i]);
            } else if (n > 0) {
                rest.push(searchdata[0][i]);
            }
        }
        ...
        for (var i = 0; i < searchdata[1].length; i++) {
            n = searchdata[1][i][0].toLowerCase().indexOf(value);
            if (!n) {
                best.push(searchdata[1][i]);
            } else if (n > 0) {
                rest.push(searchdata[1][i]);
            }
        }
        ...
        for (var i = 0; i < best.length; i++) {
            ...
            if (member) {
                ...
                // We're a member with a 2nd array map
                if (member && typeof(member[1]) != "string") {

                    var memberarr  = member[1],
                        cname      = searchdata[0][memberarr[0]],
                        type       = memberarr[1],
        ...
        while (result = rx.exec(searchList)) {
            //console.log(result[2], searchIndex[result[1]]);
            item = {
                searchValue: value,
    ...
    function prepareSearchRecords (results) {
        ...
        ExtL.each(results, function (item) {
            var searchMatch = item.searchMatch,
                ...
                member, memberType, memberName, access, targetClassName, classSuffix, types;
            ...
            // prioritize members
                ...
                if (memberName.search(searchValue, 'i') === 0) {
                    // configs
                    if (memberType === 'c') {
    ...
    function loadSearchPage (page) {
        var i = 0,
            ...
            rec, access, el, cn, privateEl, re, matchEl, href;
        ...
        for (;i < pageSize; i++) {
            ...
            if (rec) {
                ...
                if (access === 'private' || access === 'protected') {
                    cn.push({
                        class: (access === 'private' ? 'private' : 'protected') + ' mini-flag',
                        html: access === 'private' ? 'pri' : 'pro'
                    });
                }
        ...
        if (ct.childNodes.length) {
            ...
            if (ct.childNodes.length === pageSize && searchRecords.length > pageSize) {
                ct.appendChild(ExtL.createElement({
                    class: 'search-results-nav',
                    html: (pageSize * page - pageSize + 1) + ' to ' + (pageSize * page) + ' of ' + searchRecords.length,
                    cn: [{
    ...
    function onSearchKeypress (e) {
        ...
        if (keyCode == '13'){   // Enter pressed
            if (first) {
                console.log('about to follow the first item', first.href);
