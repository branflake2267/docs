/*function toggleDisplay(e) {
    var me = ExtL.get(e.target.id),
        class_string = me.value,
        class_string_active = class_string + "-active", elements;

    if (me.checked == true) {
        elements = document.querySelectorAll("." + class_string);
        swapClasses(elements, class_string, class_string_active);
    } else {
        elements = document.querySelectorAll("." + class_string_active);
        swapClasses(elements, class_string_active, class_string);
    }
}

function swapClasses(elements, remove_cls, add_cls) {
    for (var i = 0; i < elements.length; i++) {
        ExtL.removeClass(elements[i], remove_cls);
        ExtL.addClass(elements[i], add_cls);
    }
}*/
