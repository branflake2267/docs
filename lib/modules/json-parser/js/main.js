(function() {
    var hasClassTemplate = window.isClassTemplate || false,
        ...
        guideTree, tree, data, masterSearchList, searchRecords, currentPage;
    ...
    if (hasClassTemplate) {
        ExtL.get('box').onkeyup = function (e) {
            e = e || window.event,
            obj = {
                target : e.target || e.srcElement
            };
            ExtL.createBuffered(filter, 200, null)(obj);
        };
    }

    ExtL.get('guideTab').onclick = tabChange;
    ExtL.get('apiTab').onclick = tabChange;
    ExtL.get('searchtext').onkeyup = searchFilter;
    ExtL.get('searchtext').onkeydown = onSearchKeypress;
    ExtL.get('mobile-input').onkeyup = searchFilter;
    ExtL.get('mobile-input').onkeydown = onSearchKeypress;
    ExtL.get('mobile-input').onblur = onMobileInputBlur;
     ...
     */
    function tabChange() {
        if (ExtL.hasCls(this, 'active-tab')) {
            return;
        }
        ExtL.toggleCls(ExtL.get('tree'), 'hide');
        ExtL.toggleCls(ExtL.get('guide-tree'), 'hide');
        ExtL.toggleCls(ExtL.get('apiTab'), 'active-tab');
        ExtL.toggleCls(ExtL.get('guideTab'), 'active-tab');
    }
    ...
    function setTypeNavAndHeaderVisibility () {
        ...
        for (i = 0; i < headersLen; i++) {
            ...
            count = 0;
            for (j = 0; j < len; j++) {
                if (!ExtL.hasCls(els.item(j), 'be-hidden')) {
                    count++;
                    hasVisible = true;
                }
            }
    ...
    function searchFilter(e){
        e = e || window.event;
        var value = ExtL.trim((e.target || e.srcElement).value).toLowerCase(),
    ...
    function onBodyClick (e) {
        e = e || window.event;
        if (e) {
            if ((e.target || e.srcElement).id != 'searchtext') {
                hideSearchResults();
            } else {
            ...
            if (ExtL.hasCls(e.target, 'collapse-toggle') || (ExtL.hasCls(e.srcElement, 'collapse-toggle'))) {
                onMemberCollapseToggleClick(e.target);
            }
        }
    }
    ...
    /*function positionMembersBar() {
        if (!ExtL.get('member-toolbars')) {
            return false;
        }

        var membersEl = ExtL.get('rightMembers'),
            membersTop = membersEl.getBoundingClientRect().top,
            headerEl = ExtL.get('header'),
            headerHeight = headerEl.clientHeight, // add bottom border width
        ...
        toolbarsEl.style.width = membersWidth + 'px';
    ...
    function highlightTypeMenuItem() {
        ...
        for (; i < len; i++) {
            ...
            if (item.offsetParent && (itemTop < memberTypesBottom)) {
                activeItem = item;
            }
        }
    ...
    function expandTreeToClass() {
        var name = document.querySelector('.class a').innerHTML || document.querySelector('h1').innerHTML;
    ...
    function applyAceEditors () {
        ...
        for (; i < len; i++) {
            ...
            if (typeof XDomainRequest !== "undefined" || (typeof XDomainRequest !== "undefined" && typeof window.msPerformance !== "undefined")) {
    ...
    function getMemberTypeMenu () {
        ...
        if (!menu) {
            ...
            ExtL.on(menu, 'mouseenter', function () {
                menuCanClose = false;
            });

            ExtL.on(menu, 'mouseleave', function () {
                menuCanClose = true;
            });
    ...
    function showMemberTypeMenu (e) {
        ...
        var menu = getMemberTypeMenu(),
            ...
            membersBox = ExtL.get('rightMembers').getBoundingClientRect(),
            vpSize = getViewportSize(),
            height = vpSize.height - (targetBox.bottom + 40),
            maxWidth = membersBox.right - membersBox.left - 40,
            ...
            cols = [],
            tallest = 0,
            configsCt, rows, maxCols, maxLiteralWidth, useMembersWidth, width, left,
            colCount, rowCount, j, col, explicitAccessors;
        ...
        ExtL.each(memberList, function (item) {
            ...
            if (!ExtL.hasCls(item, 'be-hidden')) {
                link = item.querySelector('.member-name');
                memberObj = {
                    tag: 'a',
                    html: link.textContent || link.innerText,
                    title: ExtL.trim(link.textContent || link.innerText),
                ...
                eligMembers.push(ExtL.createElement(memberObj));
            }
        });

        if (targetId === 'methods' || targetId === 'properties') {
            eligMembers.sort(function (a, b) {
                var aStr = ExtL.trim(a.firstChild.textContent || a.firstChild.innerText || a.firstChild.nodeValue),
                    bStr = ExtL.trim(b.firstChild.textContent || b.firstChild.innerText || b.firstChild.nodeValue);
    ...
    function initMemberTypeMouseoverHandlers() {
        ...
        for (; i < len; i++) {
            ExtL.on(btns.item(i), 'mouseenter', function () {
                menuCanClose = false;
            });

            ExtL.on(btns.item(i), 'mouseleave', function () {
                menuCanClose = true;
            });

            ExtL.on(btns.item(i), 'click', function () {
                menuCanClose = true;
                hideMemberTypeMenu();
            });
