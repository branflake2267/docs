...
class SourceGuides extends SourceApi {
        ...
        for (var i = 0; i < len; i++) {
            ...
            try {
                if (CompareVersions(dirs[i], version) <= 0) {
                    console.log('\tguideDirPaths add version=' + version);
                    paths.push(Path.join(me.guidePath, dirs[i]));
                }
            } catch (e) {
                console.error('Error building guides paths for versions dirs[i]=' + dirs[i] + ' version=' + version);
                console.error('\t The directory should follow semver. Like 1.1 or 1.1.0 or 1.1.0-alpha.');
            }
        }
    ...
    mapFiles (sourceDir, path, dir, map) {
        // console.log("###@@@ mapFiles sourceDir=" + sourceDir + " path=" + path + " dir=" + dir);
    ...
    processGuides () {
        ...
        console.log('~~~~~~~~~~~~~~~~~~~~');
        console.log('~~~~~~~~~~~~~~~~~~~~');
        console.log('\t Processing Guides Start');
        ...
        console.log('\t Processing Guides End');
        console.log('~~~~~~~~~~~~~~~~~~~~');
        console.log('~~~~~~~~~~~~~~~~~~~~');
    }
    ...
    processGuideCfg () {
        return new Promise((resolve, reject) => {
            ...
            console.log('\t --- Process Guides Cfg cfg=' + cfg);

            for (; i < len; i++) {
                let guidesObj = items[i];

                console.log('\t --- * Process Guides  guidesObj.rootPath=' +  guidesObj.rootPath);
                if (guidesObj.rootPath == null) {
                    console.log('\t\t --- ROOT: ' + guidesObj.text);
                    //continue;
                }
    ...
    prepareGuides (nodes, rootPath, toReadArr, navTreeName) {
        ...
        for (var i = 0; i < nodes.length; i++) {
            ...
            if (Array.isArray(node)) {
                console.error("~~~~~~~~~~~~~~~~~~~~~~~~~~");
                console.error("Error: The node has been defined as an array, and this will not work here.");
                console.error("Suggested Fix: Consider using the children property. And remove the array config.");
                console.error("\t rootPath=" + rootPath);
                console.error("\t Skipping this node. node=", node);
                console.error("~~~~~~~~~~~~~~~~~~~~~~~~~~");
                return;
            }
            ...
            if (slug == null) {
                console.error("~~~~~~~~~~~~~~~~~~~~~~~~~~");
                console.error("Error: node.slug is undefined.");
                console.error("\t rootPath=" + rootPath);
                console.error("\t Suggested Fix: Verify the slug is defined in the json config.");
                console.error("\t Skipping this node. node=", node);
                console.error("~~~~~~~~~~~~~~~~~~~~~~~~~~");
                return;
            }
    ...
    readGuide (node) {
        return new Promise ((resolve, reject) => {
            ...
            if (path) {
                ...
                console.error("~~~~~~~~~~~~~~~~~~~~~~~~~~");
                console.error("ERROR: The guide path is undefined. path=" + path);
                console.error("\t node.id=" + node.id);
                console.error("\t node.name=" + node.name);
                console.error("\t node.slug=" + node.slug);
                console.error("\t guidePathMap=", this.guidePathMap);
                console.error("~~~~~~~~~~~~~~~~~~~~~~~~~~");

                reject("ERROR: The guide path is undefined. node.name=" + node.name);
            }
        })
        .catch(this.error.bind(this));
    }
