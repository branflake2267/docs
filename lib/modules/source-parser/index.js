...
class Source extends Base {
    ...
    beforeExecute (fileArray) {
        new compressor.minify({
            ...
            fileOut : path.normalize(this.destination + 'src' + '/css/styles.css')
        });
    }
    ...
    run () {
        let me = this,
            ...
            output     = path.join(me.destination, version) + '/',
            allCls     = require('../../' + this.input +  me.allClasses),
            srcOutput  = output + 'src/',
            files      = allCls.files,
            items      = (allCls) ? allCls.global.items : null,
            bPaths     = false,
            filePaths  = [], imagePath, localPrim, localSDK, localPkg, localSpace;

        if (fs.existsSync('./configs/basePaths.json')) {
            debug.info('There are base paths specified, have at it Team City.');

            let basePaths = JSON.parse(fs.readFileSync('./configs/basePaths.json', 'utf-8'));

            localPrim = basePaths.localPrim;
            localSDK  = basePaths.localSDK + basePaths.extFolder;
            localPkg  = basePaths.localSDK + basePaths.pkgFolder;
            imagePath = (me.imagePath) ? (basePaths.localSDK + 'docs/resources') : null;

            bPaths    = true;
        } else {
            debug.info('There are no base paths specified.  Default mode engaged.');
        ...
        files.forEach(function(file,idx) {
            ...
            if (bPaths) {
                ...
                floc  = floc.replace('../../ext', '../modules/SDK/ext').replace('../../packages','../modules/SDK/packages');
